...
function debounce<T extends (...args: any[]) => void>(func: T, wait: number) { // eslint-disable-line @typescript-eslint/no-explicit-any
  let timeout: number;
...
class Media {
  ...
  onResize({ screen, viewport }: { screen?: ScreenSize; viewport?: Viewport } = {}) {
    ...
    // Conditional scaling for mobile responsiveness
    const isMobile = this.screen.width < 768;
    const baseScaleY = isMobile ? 800 : 1400;
    const baseScaleX = isMobile ? 600 : 1100;

    this.plane.scale.y = (this.viewport.height * (baseScaleY * this.scale)) / this.screen.height;
    this.plane.scale.x = (this.viewport.width * (baseScaleX * this.scale)) / this.screen.width;
